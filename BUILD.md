...
*   **C2 服务器 (Server)**:
    *   架构: **Linux (ARM 架构)** （使用AMD64需要修改编译脚本）
...
1.  `build/server` (编译好的 Linux ARM 程序)上传到例如/root/c2
2.  `web/index.html` (Web 控制台前端)上传到例如/root/c2/web
3.  `payload.bin` (上一步生成的 Shellcode)上传到例如/root/c2/web
```
c2/
|-----server
|____web/
